...
class PluginUpdateChecker_3_0 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, __('Check for updates', 'uix-contact'));
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = __('This plugin is up to date.', 'uix-contact');
			} else if ( $status == 'update_available' ) {
				$message = __('A new version of this plugin is available.', 'uix-contact');
			} else {
				$message = sprintf(__('Unknown update checker status "%s"', 'uix-contact'), htmlentities($status));
			}
			printf(
